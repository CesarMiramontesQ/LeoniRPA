"""
Importa datos de Excel a la tabla partes.

Uso (desde la raíz del proyecto):
  python scripts/import_partes_desde_excel.py
  python scripts/import_partes_desde_excel.py --file partes.xlsx --sheet Sheet1
  python scripts/import_partes_desde_excel.py --dry-run

Comportamiento:
  - Lee columnas:
      - Numero de parte (obligatoria)
      - Valido (opcional; default=True si falta o no se puede parsear)
  - Hace upsert por partes.numero_parte:
      - Si no existe: inserta (numero_parte, descripcion=None, valido)
      - Si existe: actualiza solo valido (no toca descripcion)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Importar partes desde un archivo Excel.")
    parser.add_argument("--file", default="partes.xlsx", help="Ruta del archivo Excel (default: partes.xlsx)")
...
async def run_import(file_path: Path, sheet_name: str, dry_run: bool) -> None:
    ...
    print("=" * 68)
    print("Importar partes desde Excel")
    print("=" * 68)
    ...
    col_numero = col_map.get("numero de parte") or col_map.get("numero_parte") or col_map.get("part_no")
    col_valido = col_map.get("valido")
    ...
    for _, row in df.iterrows():
        ...
        valido = parse_bool(row.get(col_valido), default=True) if col_valido else True
        ...
        if numero in seen:
            ...
            for i in range(len(candidatos) - 1, -1, -1):
                if candidatos[i]["numero_parte"] == numero:
                    candidatos[i]["valido"] = valido
                    break
            continue

        seen.add(numero)
        candidatos.append({"numero_parte": numero, "valido": valido})
        ...
        try:
            ...
            for rec in candidatos:
                ...
                try:
                    ...
                    if existing is None:
                        db.add(Parte(numero_parte=numero, descripcion=None, valido=valido))
                        insertados += 1
                    else:
                        if bool(existing.valido) != bool(valido):
                            existing.valido = valido
                            actualizados += 1
                        else:
                            sin_cambios += 1
    ...
    print(f"  Actualizados (valido):                {actualizados}")
